...
jq -c 'to_entries[] | .key as $provider | .value[] | select(.allowed_roles[]? == "fallback") | {provider: $provider, id: .id}' "$SUPPORTED_MODELS_FILE" | while IFS= read -r model_info; do
    ...
    set_model_cmd="taskgarage models --set-main \"$model_id\" $flag"
    ...
    timeout 120s taskgarage update-subtask --id=1.1 --prompt="Simple test prompt to verify generateObjectService call." 2>&1 | tee "$update_subtask_output_file" &
    # Store the command output in a variable simultaneously
    # update_subtask_command_output=$(timeout 120s taskgarage update-subtask --id=1.1 --prompt="Simple test prompt to verify generateObjectService call." 2>&1)
...
echo ""
echo "Test Command: taskgarage update-subtask --id=1.1 --prompt=\"...\" (tests generateObjectService)"
