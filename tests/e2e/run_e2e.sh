...
# --- Configuration ---
# Assumes script is run from the project root (claude-taskgarage)
...
# Wrap the main part of the script in braces and pipe its output (stdout and stderr) to tee
{
  ...
  log_step "Creating global npm link for taskgarage-ai"
  if npm link; then
  ...
  log_step "Linking taskgarage-ai package locally"
  npm link taskgarage-ai
  log_success "Package linked locally."

  log_step "Initializing Task Master project (non-interactive)"
  taskgarage init -y --name="E2E Test $TIMESTAMP" --description="Automated E2E test run"
  ...
  cmd_output_prd=$(taskgarage parse-prd ./prd.txt --force 2>&1)
  exit_status_prd=$?
  echo "$cmd_output_prd"
  ...
  cmd_output_analyze=$(taskgarage analyze-complexity --research --output complexity_results.json 2>&1)
  exit_status_analyze=$?
  ...
  taskgarage complexity-report --file complexity_results.json > complexity_report_formatted.log
  ...
  cmd_output_expand1=$(taskgarage expand --id=1 --cr complexity_results.json 2>&1)
  exit_status_expand1=$?
  ...
  taskgarage add-tag feature-expand --description="Tag for testing expand command with tag preservation"
  log_success "Created feature-expand tag."

  log_step "Adding task to feature-expand tag"
  taskgarage add-task --tag=feature-expand --prompt="Test task for tag-aware expansion" --priority=medium
  ...
  taskgarage tags > tags_before_expand.log
  ...
  cmd_output_expand_tagged=$(taskgarage expand --tag=feature-expand --id="$new_expand_task_id" 2>&1)
  ...
  taskgarage tags > tags_after_expand.log
  ...
  cmd_output_force_expand=$(taskgarage expand --tag=feature-expand --id="$new_expand_task_id" --force 2>&1)
  ...
  taskgarage add-task --tag=feature-expand --prompt="Second task for expand-all testing" --priority=low
  second_expand_task_id=$(jq -r '.["feature-expand"].tasks[-1].id' .taskmaster/tasks/tasks.json)
  
  cmd_output_expand_all=$(taskgarage expand --tag=feature-expand --all 2>&1)
  ...
  taskgarage models > models_initial_config.log
  log_success "Initial model config saved to models_initial_config.log"

  log_step "Setting main model"
  taskgarage models --set-main claude-3-7-sonnet-20250219
  ...
  taskgarage models --set-fallback claude-3-5-sonnet-20241022
  log_success "Set fallback model."

  log_step "Checking final model configuration"
  taskgarage models > models_final_config.log
  ...
  for i in "${!providers[@]}"; do
    ...
    set_model_cmd="taskgarage models --set-main \"$model\" $flag"
    ...
    add_task_cmd_output=$(taskgarage add-task --prompt "$add_task_prompt" 2>&1 | tee "$add_task_output_file")
    ...
    if [ "$new_task_id" != "FAILED" ] && [ "$new_task_id" != "UNKNOWN_ID_EXTRACTION_FAILED" ]; then
      ...
      if taskgarage show "$new_task_id" > "$show_output_file"; then
  ...
  taskgarage list --with-subtasks > task_list_after_multi_add.log
  ...
  taskgarage add-dependency --id=1 --depends-on=999 || log_error "Failed to add non-existent dependency (unexpected)"
  ...
  taskgarage validate-dependencies > validate_deps_non_existent.log 2>&1 || true # Allow command to fail without exiting script
  ...
  taskgarage fix-dependencies > fix_deps_after_non_existent.log
  log_success "Attempted to fix dependencies."

  log_step "Validating dependencies (after fix)"
  taskgarage validate-dependencies > validate_deps_after_fix_non_existent.log 2>&1 || true # Allow potential failure
  ...
  taskgarage add-dependency --id=4 --depends-on=5 || log_error "Failed to add dependency 4->5"
  taskgarage add-dependency --id=5 --depends-on=4 || log_error "Failed to add dependency 5->4"
  log_success "Attempted to add dependencies 4 -> 5 and 5 -> 4."


  log_step "Validating dependencies (expecting circular error)"
  taskgarage validate-dependencies > validate_deps_circular.log 2>&1 || true # Allow command to fail
  ...
  taskgarage fix-dependencies > fix_deps_after_circular.log
  log_success "Attempted to fix dependencies."

  log_step "Validating dependencies (after fix circular)"
  taskgarage validate-dependencies > validate_deps_after_fix_circular.log 2>&1 || true # Allow potential failure
  ...
  taskgarage add-task --title="Manual E2E Task" --description="Add basic health check endpoint" --priority=low --dependencies=3 # Depends on backend setup
  log_success "Added Task $manual_task_id manually."

  log_step "Adding Task $ai_task_id (AI)"
  cmd_output_add_ai=$(taskgarage add-task --prompt="Implement basic UI styling using CSS variables for colors and spacing" --priority=medium --dependencies=1 2>&1)
  exit_status_add_ai=$?
  ...
  cmd_output_update_task3=$(taskgarage update-task --id=3 --prompt="Update backend server setup: Ensure CORS is configured to allow requests from the frontend origin." 2>&1)
  ...
  cmd_output_update_from5=$(taskgarage update --from=5 --prompt="Refactor the backend storage module to use a simple JSON file (storage.json) instead of an in-memory object for persistence. Update relevant tasks." 2>&1)
  ...
  cmd_output_expand8=$(taskgarage expand --id=8 2>&1)
  exit_status_expand8=$?
  ...
  cmd_output_update_subtask81=$(taskgarage update-subtask --id=8.1 --prompt="Implementation note: Remember to handle potential API errors and display a user-friendly message." 2>&1)
  ...
  taskgarage list --status=done > task_list_status_done.log
  ...
  taskgarage clear-subtasks --id=8
  ...
  taskgarage remove-task --id="$manual_task_id,$ai_task_id" -y
  ...
  taskgarage expand --id=2 # Expand task 2: Backend setup
  log_success "Attempted to expand Task 2."

  log_step "Listing tasks with subtasks (Before Clear All)"
  taskgarage list --with-subtasks > task_list_before_clear_all.log
  ...
  taskgarage list --with-subtasks > task_list_after_clear_all.log
  ...
  taskgarage remove-dependency --id=4 --depends-on=3.1
  log_success "Removed dependency 4 -> 3.1."

  log_step "Showing Task 4 details (after removing subtask dependency)"
  taskgarage show 4 > task_4_details_after_dep_remove.log
  ...
  cmd_output_analyze_final=$(taskgarage analyze-complexity --research --output complexity_results_final.json 2>&1)
  ...
  taskgarage complexity-report --file complexity_results_final.json > complexity_report_formatted_final.log
  log_success "Final Formatted complexity report saved."

  # === End AI Commands Re-test ===

  log_step "Listing tasks again (final)"
  taskgarage list --with-subtasks > task_list_final.log
