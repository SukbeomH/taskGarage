...
const registerInitializeProjectTool = (server) => {
	server.addTool({
		...
		description:
			"Initializes a new Task Master project structure in the current working directory by running 'taskgarage init'.",
		parameters: mockZod,
		execute: async (args, { log }) => {
			try {
				...
				let command = 'npx taskgarage init';
				const cliArgs = [];
				if (args.projectName) {
...
describe('Initialize Project MCP Tool', () => {
	...
	test('constructs command with proper arguments', async () => {
		...
		// Check that the command includes npx taskgarage init
		expect(command).toContain('npx taskgarage init');
